export const loginInformationChangeSelector = (state) => state.loginReducer.information;
export const registerInformationChangeSelector = (state) => state.registerReducer.information;
export const accoutInformationSelector = (state) => state.accoutReducer.information;
export const todoInformationChangeSelector = (state) => state.todoReducer.information;
export const filterInformationChangeSelector = (state) => state.filterReducer.information;
export const showFilterSelector = (state) => state.showFilterReducer.information;
export const listTaskNavigateChangeSelector = (state) => state.listTaskNavigateReducer.information;
export const showOverviewSelector = (state) => state.overviewReducer.information;