...
const StatusSelector = () => {
    ...
    return (
        <div className='flex flex-row items-center px-3 rounded-md mx-3 shadow-sm shadow-slate-400 border-[1px] border-slate-300 relative'>
            <span className='font-semibold text-[15px]'>Status</span>
            <div className='w-[1px] h-4 rounded-md mx-2 bg-black-primary'></div>
            <div className={`w-[100px] h-[30px] bg-transparent font-bold text-[15px] text-green-primary cursor-pointer flex flex-row justify-between items-center`} onClick={() => { showFilterData.status === '' ? setShowListOptions(false) : setShowListOptions(true); }}>
                <span>{filterData.status}</span>
                <img className='w-[10px] h-[10px]' src={DopdownIcon} alt="" />
                <div className={`z-10 absolute bg-green-secondary shadow-md shadow-slate-500 border-[1px] border-slate-300 w-full h-fit px-[7px] py-2 rounded-lg top-10 left-0 transition origin-top  ${showFilterData.status === '' ? 'scale-100' : 'scale-0'}`}>
